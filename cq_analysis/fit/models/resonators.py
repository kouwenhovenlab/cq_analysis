...
class HangerModel(lmfit.Model):
    ...
    def s21_khalil(fs, f0, Q_i, Q_e_mag, Q_e_phase):
        ...
        Q_c = 1.0 / ((1.0 / Q_e).real)
        Q_tot = 1.0 / (1.0 / Q_i + 1.0 / Q_c)
        return 1.0 - (Q_tot / Q_e_mag * np.exp(1j * Q_e_phase)) / (
            1 + 2j * Q_tot * (fs - f0) / f0
        )
    ...
    def func(
        ...
        sig *= amp_offset * (1.0 + amp_slope * (fs - f0) / f0)
    ...
    def guess_logic(data, fs, edge_fracs=(20, 20)):
        ...
        mag_avg = (np.abs(avg0) + np.abs(avg1)) / 2.0
        ...
        noutliers = np.where(
            (mag < (mag_avg - 3.0 * mag_std)) | (mag > (mag_avg + 3.0 * mag_std))
        )[0].size
        fracoutliers = 0.5 * noutliers / fs.size
        fwhm = np.abs(fs[-1] - fs[0]) * fracoutliers
        Ql = f0 / fwhm
        Qi = Ql / mag_min
        Qc = Ql / np.abs(1.0 - mag_min)
        ...
        phase_winding = phase_diffs[np.abs(phase_diffs) < np.pi / 8.0].mean()
...
class HangerReflectionModel(lmfit.Model):
    ...
    def s21_modified_khalil(fs, f0, Q_i, Q_e_mag, Q_e_phase):
        ...
        Q_c = 1.0 / ((1.0 / Q_e).real)
        Q_tot = 1.0 / (1.0 / Q_i + 1.0 / Q_c)
        return 1.0 - 2 * (Q_tot / Q_e_mag * np.exp(1j * Q_e_phase)) / (
            1 + 2j * Q_tot * (fs - f0) / f0
        )
    ...
    def guess(self, data, fs, edge_fracs=(20, 20)):
        ...
        # look at the average derivative at the edges, and ignore obvious jumps
        phase_diffs = np.append(np.diff(phi)[: npts // e0], np.diff(phi)[-npts // e1 :])
        phase_winding = phase_diffs[np.abs(phase_diffs) < np.pi / 2.0].mean()
...
class ResonatorReflectionWithAmp(lmfit.Model):
    ...
    def s11_with_amp(fs, f0, Qi, Qe, a, g, fcav, phi):
        ...
        nominator = (
            ...
            * a
            * np.sqrt(1 - g**2)
            * np.exp(-1j * 2 * np.pi * fs / fcav + phi * 1j)
            * s11
        )
        denominator = (
            1
    ...
    def func(
        ...
        sig *= amp_offset * (1.0 + amp_slope * fs)
...
class HangerReflectionModel_IQoffset(lmfit.Model):
    ...
    @staticmethod
    def s21_modified_khalil(fs, f0, Q_i, Q_e_mag, Q_e_phase, I_offset, Q_offset):
        Q_e = Q_e_mag
        Q_c = 1.0 / ((1.0 / Q_e).real)
        Q_tot = 1.0 / (1.0 / Q_i + 1.0 / Q_c)
        S21 = 1.0 - 2 * (Q_tot / Q_e_mag * np.exp(1j * Q_e_phase)) / (
            1 + 2j * Q_tot * (fs - f0) / f0
        )
    ...
    def func(
        ...
        sig = HangerReflectionModel_IQoffset.s21_modified_khalil(
            fs, f0, Q_i, Q_e_mag, Q_e_phase, I_offset, Q_offset
        )
        sig *= amp_offset * (1.0 + amp_slope * (fs - f0) / f0)
        sig *= np.exp(1j * (phase_offset + phase_winding * fs))
        return sig
    ...
    def guess(self, data, fs, edge_fracs=(20, 20)):
        ...
        p0["Q_i"].set(min=0)
        p0["Q_e_mag"].set(min=0)
        p0["amp_offset"].set(min=0)
        return p0
